...
export default function AddPage(){
  ...
  const [category, setCategory] = useState<string>(data.categories.expense[0]);
  const [amount, setAmount] = useState<number>(0);
  const [currency, setCurrency] = useState<Cur>('SSP');
  const [date, setDate] = useState<string>(new Date().toISOString().slice(0,10));
  const [note, setNote] = useState('');

  // Keep the user's selected category unless invalid for the chosen type
  useEffect(() => {
    const valid = type === 'income' ? data.categories.income : data.categories.expense;

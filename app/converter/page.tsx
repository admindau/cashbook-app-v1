    ...
    <div className="grid md:grid-cols-2 gap-6">
      <div className="card">
        <div className="grid gap-3">
          <label className="block"><span className="text-sm text-neutral-300">Amount</span>
            <input className="input mt-1" type="number" value={amount} onChange={e=>setAmount(parseFloat(e.target.value||'0'))}/>
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block"><span className="text-sm text-neutral-300">From</span>
            ...
            <label className="block"><span className="text-sm text-neutral-300">To</span>
          ...
          <div className="mt-2 text-lg font-semibold">Result: {Number.isFinite(result)? `${symbolFor(to)} ${result.toLocaleString(undefined,{maximumFractionDigits:2})}` : '-'}</div>
        </div>
      </div>
      <div className="card">
        <h3 className="text-lg font-semibold mb-2">Current Rates</h3>
        <ul className="text-sm text-neutral-300 space-y-1">
          <li>1 USD = {data.rates.usd_to_ssp} SSP</li>
          <li>1 KES = {data.rates.kes_to_ssp} SSP</li>
        </ul>

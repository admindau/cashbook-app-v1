...
export default function SettingsPage(){
  ...
  const saveRates=()=>{
    if(usdToSsp<=0||kesToSsp<=0){ alert('Rates must be greater than 0.'); return; }
    data.rates.usd_to_ssp=usdToSsp; data.rates.kes_to_ssp=kesToSsp; save(data); toast.show('Exchange rates updated successfully.');
  };
  return (<Shell>
    ...
    <div className="grid md:grid-cols-2 gap-6">
      <div className="card">
        ...
        <div className="grid gap-3">
          <label className="block"><span className="text-sm">1 USD = (SSP)</span>
            <input className="input mt-1" type="number" value={usdToSsp} onChange={e=>setUsdToSsp(parseFloat(e.target.value||'0'))}/>
          </label>
          <label className="block"><span className="text-sm">1 KES = (SSP)</span>
    ...
    <Modal isOpen={showModal} onClose={()=>setShowModal(false)} onConfirm={onReset} title="Reset Storage?" message="Are you sure you want to reset all storage? This will delete your records permanently." confirmText="Delete" cancelText="Cancel" variant="danger"/>
  </Shell>);
}

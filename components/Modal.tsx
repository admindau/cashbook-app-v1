...
export default function Modal({ isOpen, onClose, onConfirm, title, message, confirmText='Confirm', cancelText='Cancel', variant='default', children }: Props){
  if (!isOpen) return null;
  const base = 'px-4 py-2 rounded-lg text-white';
  const confirm = variant==='danger' ? `${base} bg-red-600` : variant==='success' ? `${base} bg-green-600` : `${base} bg-neutral-700`;
  return (
